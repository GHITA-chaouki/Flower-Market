...
namespace backend.Admin
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        // ======================
        [HttpGet("products")]
        public async Task<IActionResult> GetAllProducts([FromQuery] string? prestataireId = null)
        {
            try
            {
                var products = await _adminService.GetAllProducts(prestataireId);
                return Ok(new
                {
                    success = true,
        ...
        // ======================
        // GET PRODUCTS BY PRESTATAIRE
        // ======================
        [HttpGet("prestataires/{prestataireId}/products")]
        public async Task<IActionResult> GetProductsByPrestataire(string prestataireId)
        {
            try
            {
                var products = await _adminService.GetAllProducts(prestataireId);
            ...
            catch (Exception ex)
            {
                ...
                {
                    success = false,
                    message = $"Erreur lors de la récupération des produits du prestataire {prestataireId}",
                    error = ex.Message
                });
            }
        }
